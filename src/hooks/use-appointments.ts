...
export default function useAppointments({ filters }: Props = { filters: {} }) {
  ...
  const [ morningAppointments, afternoonAppointments, nightAppointments, usedTimeSlots ] = React.useMemo(() => {
  const  [ morningAppointments, afternoonAppointments, nightAppointments, usedTimeSlots ] = appointments.reduce<
      [ AppointmentFormatted[], AppointmentFormatted[], AppointmentFormatted[], string[] ]>(
      (acc, appointment) => {
